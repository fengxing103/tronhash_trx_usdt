...
class User extends Frontend
{
    ...
    public function subordinate(){
        ...
        $list = Db::name("user")->where("pid",$user['id'])->select();
        $rlist = [];
        foreach ($list as $key){
            
            $key['trx_count'] = Db::name("game_record")->where("owner_address",$key['email'])->count();
            $key['trx_sum'] = Db::name("game_record")->where("owner_address",$key['email'])->sum("amount");
            $key['usdt_count'] = Db::name("game_record")->where("owner_address",$key['email'])->count();
            $key['usdt_sum'] = Db::name("game_record")->where("owner_address",$key['email'])->sum("amount");
            $key['trx_profit'] = Db::name("transfer_record")->where("to_address",$key['email'])->sum("amount");
            $key['usdt_profit'] = Db::name("transfer_record")->where("to_address",$key['email'])->sum("amount");
            
            array_push($rlist,$key);
        }
      
        $this->view->assign('list',$rlist);
        $this->view->assign('title',"下级列表");
        return $this->view->fetch();
    }
    //佣金记录
    ...
    public function Commissionrecord(){
        ...
        $list = Db::name("game_commission")->where("to_address",$user['email'])->paginate(10,true);
        $this->view->assign('list',$list);
        $this->view->assign('title',"佣金记录");
        return $this->view->fetch();
